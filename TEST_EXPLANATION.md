...
The `ConcurrentHitCounter` is a lock-free, thread-safe implementation of a hit counter that uses **atomic operations**
and **Compare-And-Set (CAS)** to handle concurrent access without traditional locks. This test suite validates both
functional correctness and thread-safety guarantees.
...
**Why this matters**: This is a **stress test** for the CAS mechanism. High contention causes many CAS failures, testing
retry logic.
...
**Why this matters**: Race conditions are **non-deterministic**—they may not appear in every run. Repeating tests
...
The `Deduplicator` (single-threaded) and `ConcurrentDeduplicator` (thread-safe) implement TTL-based deduplication using
a HashMap with a ring of HashSet buckets. Tests verify time-based expiration, bucket management, and thread-safety.
...
The `LRU` (single-threaded) and `ConcurrentLRU` (thread-safe) implement Least Recently Used cache with O(1) get/put
using HashMap and doubly linked list. Tests verify eviction, access order, and thread-safety.
