...
/**
 * A thread-safe TTL-based deduplicator that reports whether a key is being seen
 * for the first time within a configurable time-to-live window.
 * <p>
 * This class maintains two coordinated data structures:
 * <ul>
 *   <li>A {@link ConcurrentHashMap} mapping each key to the epoch second at which it was last first-seen</li>
 *   <li>A ring of concurrent {@link Set} buckets (one per second of TTL) used by {@link #sweeper()}
 *       to expire keys incrementally without scanning the whole cache</li>
 * </ul>
 * <p>
 * <b>Concurrency Model:</b>
 * Relies on {@link ConcurrentHashMap#compute(Object, java.util.function.BiFunction)} to atomically
 * check-and-update a key's last-seen timestamp, guaranteeing exactly one caller observes
 * {@code true} per key per TTL window. Bucket membership is updated inside the compute block,
 * which executes under the map's per-bin lock for that key.
 * <p>
 * <b>Design Decisions:</b>
 * <ul>
 *   <li><b>Atomic compute:</b> Avoids check-then-act races without a global lock,
 *       allowing high concurrency across distinct keys.</li>
 *   <li><b>Bucketed expiry:</b> Keys are placed in the bucket {@code timestamp % ttl}, so the
 *       sweeper only needs to clear the buckets that have elapsed since the last sweep.</li>
 *   <li><b>Conditional remove in sweeper:</b> {@code cache.remove(key, insertedAt)} only removes
 *       entries whose timestamp matches the expiring bucket, so refreshed keys survive.</li>
 *   <li><b>Single-threaded sweeper:</b> {@link #sweeper()} must only be called from one thread
 *       (e.g. a single-thread ScheduledExecutorService); {@code lastSweptSecond} is volatile
 *       for visibility, not mutual exclusion.</li>
 *   <li><b>Injectable {@link Clock}:</b> Enables deterministic testing of time-dependent behavior.</li>
 * </ul>
 *
...
public class ConcurrentDeduplicator<K> {
    ...
    /** Must only be called from a single thread. Schedule via a single-thread ScheduledExecutorService. */
    public void sweeper(){
