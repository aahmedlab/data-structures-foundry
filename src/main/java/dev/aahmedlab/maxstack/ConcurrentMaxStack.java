...
/**
 * A thread-safe implementation of a max stack that supports O(log n) retrieval of the maximum element.
 * <p>
 * This class maintains two synchronized data structures:
 * <ul>
 *   <li>A {@link DoubleLinkedList} representing the stack order</li>
 *   <li>A {@link TreeMap} mapping values to their occurrences for O(log n) max retrieval</li>
 * </ul>
 * <p>
 * <b>Concurrency Model:</b>
 * Uses a single private lock object with intrinsic synchronization ({@code synchronized}) on all public methods.
 * This ensures a happens-before relationship between reads and writes, preventing stale data visibility.
 * <p>
 * <b>Design Decisions:</b>
 * <ul>
 *   <li><b>Single lock:</b> The invariant that elements appear in both structures simultaneously
 *       requires coordinated access, so per-structure locking would be insufficient.</li>
 *   <li><b>Intrinsic synchronization:</b> Chosen over {@link java.util.concurrent.locks.ReentrantLock}
 *       for simplicity—no timeout, fairness, or condition variables are needed.</li>
 *   <li><b>Private lock object:</b> Lock encapsulation prevents external code from acquiring the monitor,
 *       avoiding potential deadlocks or interference.</li>
 *   <li><b>Unsynchronized DoubleLinkedList:</b> The underlying list is private and only accessed
 *       under the class's lock, so additional synchronization is unnecessary.</li>
 * </ul>
 *
