...
/**
 * A single-threaded max stack that supports {@code popMax} in O(log n) by removing the maximum
 * element from anywhere in the stack.
 *
 * <p>This class maintains two coordinated data structures:
 *
 * <ul>
 *   <li>A {@link DoubleLinkedList} representing the stack order
 *   <li>A {@link TreeMap} mapping values to a {@link Deque} of their nodes, with the most recent
 *       occurrence on top, for O(log n) max retrieval and removal
 * </ul>
 *
 * <p><b>Design Decisions:</b>
 *
 * <ul>
 *   <li><b>Linked list over array stack:</b> {@code popMax} must unlink an element from the middle
 *       of the stack; a doubly linked list does this in O(1) given the node.
 *   <li><b>TreeMap of node deques:</b> {@code lastKey()}/{@code lastEntry()} locate the max in
 *       O(log n), and the per-value deque resolves duplicates to the most recently pushed
 *       occurrence.
 *   <li><b>Node handles:</b> {@link DoubleLinkedList#push} returns the created {@link Node},
 *       letting the TreeMap reference list positions directly without searching.
 * </ul>
 *
 * <p><b>Thread Safety:</b> This class is <em>not</em> thread-safe. See {@link ConcurrentMaxStack}
 * for a concurrent variant (same implementation, synchronized).
 *
