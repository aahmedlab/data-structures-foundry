...
class ConcurrentDeduplicatorTest {
  ...
  void testConcurrentAccessWithExpiration() throws InterruptedException {
    ...
    assertEquals(
        threadCount, falseCount, "All threads should see key as not first seen within TTL");
  ...
  void testConcurrentCorrectnessWithExpiration() throws InterruptedException {
    ...
    assertEquals(
        1, trueCount.get(), "Second round: exactly one thread should see expired key as first");
